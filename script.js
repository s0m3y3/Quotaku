...
function dummygetCharacterQuotes() {
  ...
  // animetitle = "One Piece"; //developer input data. 
  $("#charactername").append(searchedcharacter);
...
function MAL_AnimeNameSearch() {
  ...
  const settings = {
    async: true,
    crossDomain: true,
    url: `https://myanimelist.p.rapidapi.com/anime/search/${MAL_animesearch}/10`, //MAL API Link. Search for 10 results. 
  ...
  $.ajax(settings).then(function (dataMAL) {
    ...
    animeID_title = animeTitleArray.map((title, index) => {return { title, id: animeID[index] };})  //combines 2arrays (anime title & ID) into an array object. DO NOT NEED At this time. 

    if (animeIDlist.length == 0) {
      return;     //if animeID is blank, no anime found. Stops function.
    } else {
      // console.log("anime found");
      //NOTE: For the future, Need to filter the animeIDlist, to exclude titles with "movies" or "film" as MAL data tends to lack character information. For now, will just search animeIDlist[0].
      let animeIndex = findIndexOfSmallestNumber(animeIDlist);
      MAL_IDsearch(animeIDlist[animeIndex]);
      ...
      //Filter2: If searched "ichigo" and data shows, "Kurosaki Ichigo" - separated by spacing, It will split string into two "ichigo" & "kurosaki" then, compare one at a time to the searched "ichgo." 
        for (let i = 0; i < charName.length; i++) {
          if (charName[i].toLowerCase().includes(searchedcharacter.toLowerCase())) {
            console.log("Filter#2 Success: "+ i);
            Jikan_CharacterImageSearch(charid[i]); //this function finds image of character.
            break;
          }    
          else{
            console.log("Filter#2 failed. Filter#3 is still in progress of coding");
...
button.addEventListener("click", function () {
  ...
  // dummygetCharacterQuotes(); //fake quote function. used to keep api limit fresh.
